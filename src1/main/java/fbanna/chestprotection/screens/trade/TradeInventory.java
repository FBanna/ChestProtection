...
public class TradeInventory extends SimpleContainer {
    ...
    public void dropAll() {
        ...
        for(int i = 0; i < this.getContainerSize(); i++){
            ...
            if (!stack.isEmpty()){
                ...
                boolean wasAdded = this.player.getInventory().add(stack);
    ...
    private static boolean ItemComponentEquals(ItemStack original, TypedDataComponent component) {
        for (TypedDataComponent<?> type: original.getComponents()) {
    ...
    public static boolean ItemsEqual(ItemStack stack1ORIGINAL, TradeItem tradeItemORIGINAL){
        ...
        for (TypedDataComponent<?> type: tradeStack.getStack().getComponents()) {
    ...
    public boolean canFit() {
        ...
        for(ItemStack costStack: stacks){
            ItemStack remaining = profitInventory.addItem(costStack);
    ...
    public void doTrade() {
        ...
        for(int i = 0; i < this.getContainerSize(); i++){
            if (ItemsEqual(this.getItem(i), this.trade.tradeItems.getCost())){
                costTotal += this.getItem(i).getCount();
                costSlots.add(i);
            }
        }
        ...
        if(
            ...
            for(int slot: costSlots){
                ...
                if ( costCountdown > 0 ){
                    ...
                    if(costCountdown >= stack.getCount()){
                        ...
                        this.setItem(slot, Items.AIR.getDefaultInstance());
                        ...
                        this.setItem(slot, stack.copyWithCount(stack.getCount() - costCountdown));
            ...
            for(ItemStack productStack: productStacks){
                ...
                player.drop(this.addItem(productStack), false, false);
            }
            //ChestProtection.LOGGER.info("after "+ productStacks.toString());

            for(ItemStack costStack: costStacks){
                ItemStack remaining = this.trade.profitInventory.addItem(costStack);
